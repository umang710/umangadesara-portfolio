...
const UMANG_SKILLS: Record<string, string[]> = {
  ...
  "Cloud & DevOps": ["aws", "docker", "kubernetes", "k8s", "ci/cd", "deployment"],
  "Database": ["sql", "mysql", "postgresql", "postgres", "supabase", "rdbms"],
  "Machine Learning": ["ml", "machine learning", "tensorflow", "pytorch", "nlp", "lstm", "model"],
...
export async function analyzeJobDescription(_prevState: unknown, formData: FormData) {
  const jdText = formData.get("jd")?.toString().toLowerCase();
  
  if (!jdText || jdText.length < 20) {
    return { error: "Please paste a longer job description for an accurate match." };
  }
  ...
  if (totalJdSkillsDetected === 0) {
    ...
    let baseScore = (umangSkillCount / totalJdSkillsDetected) * 100;
