...
export function JobMatcher() {
  ...
  return (
    <div className="p-6 bg-white/40 dark:bg-zinc-900/40 border border-brand-accent/20 rounded-xl backdrop-blur-sm shadow-sm">
      ...
      <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-6">
        Recruiting? Paste a job description below. The engine will run semantic
        keyword analysis to calculate alignment with my engineering stack.
      </p>
      ...
      <form ref={formRef} action={formAction} className="space-y-4">
        <textarea
          ...
          className="w-full px-4 py-3 bg-white/80 dark:bg-zinc-950/80 border border-brand-accent/20 rounded-lg text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-brand-accent/50 text-sm resize-none transition-shadow"
        />
          ...
          {state?.success && state.score !== undefined && (
            <div className="flex items-center gap-4 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="text-right">
                <span className="block text-xs text-zinc-500 dark:text-zinc-400 uppercase tracking-wider font-semibold">
                  Match Score
                </span>
                <span
                  className={`text-2xl font-bold font-mono ${getScoreColor(state.score)}`}
                >
                  {state.score}%
                </span>
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            {state.matched.length > 0 ? (
              state.matched.map((skill) => (
                <span
                  key={skill}
                  className="inline-flex items-center gap-1.5 px-3 py-1 bg-brand-accent/10 dark:bg-brand-accent/20 text-brand-accent border border-brand-accent/20 rounded-md text-xs font-semibold shadow-sm"
                >
              ...
              <span className="text-sm text-zinc-500">
                No direct technical overlap detected in this description.
              </span>
            )}
          </div>
        </div>
